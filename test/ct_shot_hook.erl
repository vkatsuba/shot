...
on_tc_fail(Suite, TC, Reason, State) ->
    io:format("on_tc_fail ~p~n", [{Suite, TC, Reason, State}]),
    State.
...
%% any common state.
init(_Id, Opts) ->io:format("AAAAAAAAAAAAAAADDDDDDDDDDDDDDDDDDDD ~p~n", [1]),
    ct:pal("AAAAAAAAAAAAAAAAAA init opts: ~p", [Opts]),
    {ok, #{total_ok => 0,
           total_failed => 0,
...
post_init_per_suite(_SuiteName, _Config, Return, State) ->
   ct:pal("AAAAAAAAAAAAAAAAAA post_init_per_suite opts: ~p", [Return]),
   {Return, State}.

post_init_per_group(_GroupName, _Config, Return, State) ->
    ct:pal("AAAAAAAAAAAAAAAAAA post_init_per_group opts: ~p", [Return]),
   {Return, State}.
...
post_end_per_group(GroupName, Config, Return, State) ->
    ...
    ct:log("NewStatexxxxxx: ~p", [State1]),
    {Return, State1}.

terminate(#{total_ok := OK, total_eventually_ok_tests := TotalEventuallyOK,
            total_failed := TotalFailed} = State) ->ct:pal("terminate terminate opts: ~p", [State]),
